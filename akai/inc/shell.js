...
function disp_imp(aspace, addr, n){
    try{
        ...
        for(var i = 0; i < n; i++){
            ...
            //CodeLion's nid resolve code:
            //num functs at addr+6
            //nid table at addr+24
            nid = parseInt(nid,16);

            var numfunc_a = maddr+6;
            var nidtable_a = maddr+24;
            var enttable_a =  maddr+28;
            var numfunc =  aspace16[numfunc_a/2];
            var nidtable = aspace32[nidtable_a/4];
            var enttable = aspace32[enttable_a/4];
            do_read(aspace,nidtable,numfunc*4);



            if (nid != 0x0){
                for(var j = 0; i<numfunc; i=i+4)
                {
                    if (aspace32[(nidtable+i)/4] == nid){
                        logdbg("nid found at: 0x" + (nidtable+i).toString(16));
                        do_read(aspace,(enttable+i),0x4);



                        //do_dis(aspace,enttable+(i*4),0x8,0x0);
...
*/ 
function disp_ex(aspace, addr, n,nid){
    try{
        var maddr = addr;
        for(var i = 0; i < n; i++){
    ...
    try{
        while(true){
            ...
            // search string
                ...
                if(cmd_s.length > 3){nid = Number(cmd_s[3]) };

                disp_ex(aspace, begaddr, n,nid);
            }
